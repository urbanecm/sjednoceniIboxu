...
class wikipedia {
     ...
     * @return void
     **/
    function __construct ($url='http://cs.wikipedia.org/w/api.php',$hu=null,$hp=null) {
        $this->http = new http;
        $this->token = null;
        $this->url = $url;
     ...
     * @return array
     **/
    function categorymembers ($category,$subcat=false,$extra=null) {
        $continue = '';
        $pages = array();
        while (true) {
            $res = $this->query('?action=query&list=categorymembers&cmtitle='.urlencode($category).'&format=php&cmlimit=500'.$continue.$extra);
            if (isset($x['error'])) {
                return false;
            }
    ...
    **/
    function whatusethetemplate ($template,$extra=null) {
        ...
        $pages = array();
        while (true) {
            $res = $this->query('?action=query&list=embeddedin&eititle=Šablona:'.urlencode($template).'&eilimit=500&format=php'.$continue.$extra);
            if (isset($res['error'])) {
                return false;
            }
    ...
    function edit ($page,$data,$summary = '',$minor = false,$bot = true,$section = null,$detectEC=false,$maxlag='') {
        ...
        return $this->query('?action=edit&assert=bot&maxlag=5&format=php'.$maxlag,$params);  //zde pridano &assert=bot&maxlag=5
    }
    ...
    **/
     ...
     * @return api result
     **/
    function move ($old,$new,$reason,$options=null, $noredirect = false) {
        if ($this->token==null) {
            $this->token = $this->getedittoken();
        }
        $params = array(
        ...
        if($noredirect) $params['noredirect'] = true;
        if ($options != null) {
        ...
        return $this->query('?action=move&movetalk&format=php',$params);
    }
